...
///
/// CoolProp's native library is not thread-safe: concurrent calls to
/// `AbstractState_factory` (and potentially other entry points) corrupt
/// internal global state.  Holding this lock for every FFI call ensures
/// correct behavior when multiple [`AbstractState`] instances are created or
/// used from different threads simultaneously.
///
/// The `CoolProp<F>` wrapper adds a second, per-instance `Mutex<AbstractState>`
/// to keep `update`/`keyed_output` pairs atomic.  Both layers are needed:
/// the global lock prevents cross-instance races; the per-instance lock
/// prevents races between trait-method call pairs on the same model.
...
impl AbstractState {
    ...
    /// outputs such as [`OutputParam::MolarMass`].
    ///
    /// # Errors
    ///
...
impl Drop for AbstractState {
    fn drop(&mut self) {
        ...
        // calls.  Errors during drop are silently discarded — there is no
        // clean way to propagate them.
        unsafe {
